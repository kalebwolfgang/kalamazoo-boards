#!/usr/bin/env python3
"""
Northside Cultural Business District Authority Board scraper
=============================================================
CivicClerk category 43 only — no YouTube.
Upcoming meetings pulled from CivicClerk future events.

Default mode (no arguments):
    Fetches the last 6 months + next 6 months for upcoming.
    python ncbda_scraper.py

Backfill mode:
    python ncbda_scraper.py --start-date 2020-01-01
"""
...
NCBDA_CATEGORY_ID  = 43
NCBDA_KEYWORDS     = ["northside cultural business district", "northside cultural", "ncbda"]
BOARD_NAME         = "Northside Cultural Business District Authority Board"
OUTPUT_PATH        = Path("data") / "ncbda.json"
...
def is_ncbda_event(event: dict) -> bool:
    if event.get("eventCategoryId") != NCBDA_CATEGORY_ID:
        return False
    name_lower = event.get("eventName", "").lower()
    return any(kw in name_lower for kw in NCBDA_KEYWORDS)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Scrape Kalamazoo NCBDA Board meetings.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  python ncbda_scraper.py                            # Default: last 6 months\n"
            "  python ncbda_scraper.py --start-date 2020-01-01   # Full backfill\n"
        ),
    )
...
def main() -> None:
    ...
    print(f"NCBDA Scraper [{mode} mode]")
    ...
    ncbda_events = [e for e in all_events if is_ncbda_event(e)]

    past_events   = [e for e in ncbda_events if e["startDateTime"].split("T")[0] <= today_iso]
    future_events = [e for e in ncbda_events if e["startDateTime"].split("T")[0] > today_iso]

    print(f"  NCBDA events found: {len(ncbda_events)} ({len(past_events)} past, {len(future_events)} upcoming)")
    print()
    ...
    # ── Step 2: Merge
    print("── Step 2: Merging with existing data/ncbda.json...")
    ...
    # ── Step 3: Upcoming from CivicClerk future events
    upcoming = events_to_upcoming(future_events, "6:00 PM \u2013 7:00 PM")
    print(f"── Step 3: {len(upcoming)} upcoming meetings from CivicClerk")
    for u in upcoming:
