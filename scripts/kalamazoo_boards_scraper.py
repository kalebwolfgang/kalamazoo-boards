"""
Kalamazoo Downtown Economic Growth Authority Meeting Scraper
=============================================================

Pulls DEGA meetings from the CivicClerk API and writes everything to
data/dega.json. No YouTube step — DEGA meetings are not streamed.

Default mode (no arguments):
    Fetches the last 6 months. Merges into existing data/dega.json.
    This is what GitHub Actions runs on a weekly cron.

    python dega_scraper.py

Backfill mode:
    Fetches everything from the given date forward.

    python dega_scraper.py --start-date 2020-01-01
"""
...
DEGA_CATEGORY_ID    = 39
DEGA_KEYWORDS       = ["downtown economic growth authority", "dega"]

LOOKBACK_MONTHS     = 6
CUTOFF_DATE         = "2020-01-01"

OUTPUT_PATH         = Path("data") / "dega.json"
...
def is_dega_event(event: dict) -> bool:
    if event.get("eventCategoryId") != DEGA_CATEGORY_ID:
        return False
    name_lower = event.get("eventName", "").lower()
    return any(kw in name_lower for kw in DEGA_KEYWORDS)
...
def normalize_meeting_type(event_name: str) -> str:
    ...
    return "Downtown Economic Growth Authority"
...
def compute_upcoming_meetings(n: int = 6) -> list[dict]:
    """Generate next N DEGA meeting dates.

    DEGA meets jointly with the DDA roughly on the 3rd Monday of every month,
    but specific dates are set by the Board and may vary. This computes the
    3rd Monday as a best estimate — manual adjustment may be needed.
    """
    today   = date.today()
    results = []
    year    = today.year
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape Kalamazoo DEGA meetings.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  python dega_scraper.py                              # Default: last 6 months\n"
            "  python dega_scraper.py --start-date 2020-01-01      # Full backfill\n"
        ),
    )
...
def main() -> None:
    ...
    print(f"DEGA Scraper [{mode} mode]")
    ...
    dega_events = [e for e in all_events if is_dega_event(e)]
    print(f"  DEGA events found: {len(dega_events)}")
    print()

    scraped = [m for m in (transform_civicclerk_event(e) for e in dega_events) if m is not None]
    for m in scraped:
    ...
    print("── Step 2: Merging with existing data/dega.json...")
