#!/usr/bin/env python3
"""
Zoning Board of Appeals scraper
================================
CivicClerk category 30 + YouTube
Upcoming meetings pulled from CivicClerk future events (reliable).

Default mode (no arguments):
    Fetches the last 6 months + next 6 months for upcoming.
    python zba_scraper.py

Backfill mode:
    python zba_scraper.py --start-date 2020-01-01
"""
...
ZBA_CATEGORY_ID   = 30
ZBA_KEYWORDS      = ["zoning board of appeals", "zoning board"]
BOARD_NAME        = "Zoning Board of Appeals"
OUTPUT_PATH       = Path("data") / "zba.json"
...
def is_zba_event(event: dict) -> bool:
    if event.get("eventCategoryId") != ZBA_CATEGORY_ID:
        return False
    name_lower = event.get("eventName", "").lower()
    return any(kw in name_lower for kw in ZBA_KEYWORDS)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Scrape Kalamazoo Zoning Board of Appeals meetings.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  python zba_scraper.py                             # Default: last 6 months\n"
            "  python zba_scraper.py --start-date 2020-01-01    # Full backfill\n"
        ),
    )
...
def main() -> None:
    ...
    zba_events = [e for e in all_events if is_zba_event(e)]

    past_events   = [e for e in zba_events if e["startDateTime"].split("T")[0] <= today_iso]
    future_events = [e for e in zba_events if e["startDateTime"].split("T")[0] > today_iso]

    print(f"  ZBA events found: {len(zba_events)} ({len(past_events)} past, {len(future_events)} upcoming)")
    print()
    ...
    # ── Step 3: Merge
    print("── Step 3: Merging with existing data/zba.json...")
