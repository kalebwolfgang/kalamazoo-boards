#!/usr/bin/env python3
"""
SPK Organizing Committee scraper
=================================
CivicClerk category 44 only — no YouTube.
Upcoming meetings are preserved from existing JSON (bimonthly schedule
...
Default mode (no arguments):
    ...
    python spk_scraper.py

Backfill mode:
    python spk_scraper.py --start-date 2020-01-01
"""
...
SPK_CATEGORY_ID   = 44
SPK_KEYWORDS      = ["shared prosperity kalamazoo", "spk organizing committee", "spk"]
BOARD_NAME        = "SPK Organizing Committee"
OUTPUT_PATH       = Path("data") / "spk.json"
...
def is_spk_event(event: dict) -> bool:
    if event.get("eventCategoryId") != SPK_CATEGORY_ID:
        return False
    name_lower = event.get("eventName", "").lower()
    return any(kw in name_lower for kw in SPK_KEYWORDS)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Scrape Kalamazoo SPK Organizing Committee meetings.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  python spk_scraper.py                            # Default: last 6 months\n"
            "  python spk_scraper.py --start-date 2020-01-01   # Full backfill\n"
        ),
    )
...
def main() -> None:
    ...
    spk_events = [e for e in all_events if is_spk_event(e)]

    past_events = [e for e in spk_events if e["startDateTime"].split("T")[0] <= today_iso]

    print(f"  SPK events found: {len(spk_events)} ({len(past_events)} past)")
    print()
    ...
    # ── Step 2: Merge
    print("── Step 2: Merging with existing data/spk.json...")
