"""
Kalamazoo Economic Development Corporation Meeting Scraper
==========================================================

Pulls EDC meetings from the CivicClerk API and writes everything to
data/edc.json. No YouTube step — EDC meetings are not streamed.

Default mode (no arguments):
    Fetches the last 6 months. Merges into existing data/edc.json.
    This is what GitHub Actions runs on a weekly cron.

    python edc_scraper.py

Backfill mode:
    Fetches everything from the given date forward.

    python edc_scraper.py --start-date 2020-01-01
"""
...
EDC_CATEGORY_ID     = 33
EDC_KEYWORDS        = ["economic development corporation", "edc"]

LOOKBACK_MONTHS     = 6
CUTOFF_DATE         = "2020-01-01"

OUTPUT_PATH         = Path("data") / "edc.json"
...
def is_edc_event(event: dict) -> bool:
    if event.get("eventCategoryId") != EDC_CATEGORY_ID:
        return False
    name_lower = event.get("eventName", "").lower()
    return any(kw in name_lower for kw in EDC_KEYWORDS)
...
def compute_upcoming_meetings(n: int = 6) -> list[dict]:
    """Generate next N EDC meeting dates.

    EDC meets jointly with the BRA on the 3rd Thursday of every month.
    The EDC meeting convenes at 7:45 AM; BRA follows immediately after.
    """
    today   = date.today()
    results = []
    year    = today.year
    ...
    while len(results) < n and checked < 24:
        third_thu = third_weekday_of_month(year, month, weekday=3)  # 3 = Thursday
        if third_thu >= today:
            results.append({
                "date":    third_thu.strftime("%Y-%m-%d"),
                "display": third_thu.strftime("%A, %B %#d, %Y"),
                "time":    "7:45 AM",
            })
        month += 1
        if month > 12:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape Kalamazoo EDC meetings.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  python edc_scraper.py                              # Default: last 6 months\n"
            "  python edc_scraper.py --start-date 2020-01-01      # Full backfill\n"
        ),
    )
...
def main() -> None:
    ...
    edc_events = [e for e in all_events if is_edc_event(e)]
    print(f"  EDC events found: {len(edc_events)}")
    print()

    scraped = [m for m in (transform_civicclerk_event(e) for e in edc_events) if m is not None]
    for m in scraped:
    ...
    print("── Step 2: Merging with existing data/edc.json...")
