"""
Kalamazoo Brownfield Redevelopment Authority Meeting Scraper
============================================================

Pulls BRA meetings from the CivicClerk API and writes everything to
data/bra.json. No YouTube step — BRA meetings are not streamed.

Default mode (no arguments):
    Fetches the last 6 months. Merges into existing data/bra.json.
    This is what GitHub Actions runs on a weekly cron.

    python bra_scraper.py

Backfill mode:
    Fetches everything from the given date forward.

    python bra_scraper.py --start-date 2020-01-01
"""
...
BRA_CATEGORY_ID     = 34
BRA_KEYWORDS        = ["brownfield redevelopment authority"]

LOOKBACK_MONTHS     = 6
CUTOFF_DATE         = "2020-01-01"

OUTPUT_PATH         = Path("data") / "bra.json"
...
def is_bra_event(event: dict) -> bool:
    if event.get("eventCategoryId") != BRA_CATEGORY_ID:
        return False
    name_lower = event.get("eventName", "").lower()
    return any(kw in name_lower for kw in BRA_KEYWORDS)
...
def normalize_meeting_type(event_name: str) -> str:
    ...
    return "Brownfield Redevelopment Authority"
...
def compute_upcoming_meetings(n: int = 6) -> list[dict]:
    """Generate next N BRA meeting dates.

    BRA meets the 3rd Thursday of every month.
    """
    today   = date.today()
    results = []
    year    = today.year
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape Kalamazoo BRA meetings.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  python bra_scraper.py                              # Default: last 6 months\n"
            "  python bra_scraper.py --start-date 2020-01-01      # Full backfill\n"
        ),
    )
...
def main() -> None:
    ...
    bra_events = [e for e in all_events if is_bra_event(e)]
    print(f"  BRA events found: {len(bra_events)}")
    print()

    scraped = [m for m in (transform_civicclerk_event(e) for e in bra_events) if m is not None]
    for m in scraped:
    ...
    print("── Step 2: Merging with existing data/bra.json...")
