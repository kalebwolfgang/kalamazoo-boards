"""
Kalamazoo Historic District Commission (HDC) Meeting Scraper
============================================================

Pulls HDC meetings from the CivicClerk API and past recordings from the
Kalamazoo City TV YouTube channel (live streams tab), then writes everything
to data/hdc.json in the format the website expects.
...
Default mode (no arguments):
    ...
    Merges new data into the existing data/hdc.json. This is what GitHub
    Actions runs on a weekly cron.

    python scripts/hdc_scraper.py
...
Backfill mode (custom start date):
    ...
    python scripts/hdc_scraper.py --start-date 2020-01-01
"""
...
HDC_CATEGORY_ID         = 35
HDC_KEYWORDS            = ["historic district commission", "historic district", "hdc"]
...
OUTPUT_PATH             = Path("data") / "hdc.json"
...
def is_hdc_event(event: dict) -> bool:
    """Return True only for genuine HDC events."""
    if event.get("eventCategoryId") != HDC_CATEGORY_ID:
        return False
    name_lower = event.get("eventName", "").lower()
    return any(kw in name_lower for kw in HDC_KEYWORDS)
...
def transform_civicclerk_event(event: dict) -> dict:
    """Turn a raw CivicClerk event into an HDC meeting record."""
    event_id  = event["id"]
    ...
    # Drop completely empty administrative shells
    if not agenda_file_id and not minutes_file_id and not cancelled:
        return None 
    ...
    return {
        ...
        "meeting_type": "Historic District Commission",
        "cancelled":   cancelled,
...
def fetch_youtube_streams(api_key: str, start_date: str, end_date: str) -> list[dict]:
    """Search the Kalamazoo City TV channel for HDC live stream recordings."""
...
def match_recordings_to_meetings(
    ...
    tolerance:  int = YOUTUBE_DATE_TOLERANCE,) -> tuple[list[dict], list[dict]]:
    ...
    # RETURN ALL RECORDINGS so the UI card is fully populated
    return meetings, recordings


# ---------------------------------------------------------------------------
# Upcoming meetings (Algorithmically Generated)
# ---------------------------------------------------------------------------

def compute_upcoming_meetings(n: int = 6) -> list[dict]:
    """
    Generate the next N HDC meeting dates.
    HDC meets the 3rd Tuesday of each month.
    """
    today = date.today()
    results = []
    ...
    while len(results) < n and checked < 36:
        # Find 1st day of the month
        first_of_month = date(year, month, 1)
        
        # Weekday 1 is Tuesday. Find days until the 1st Tuesday
        days_to_tuesday = 1 - first_of_month.weekday()
        if days_to_tuesday < 0:
            days_to_tuesday += 7
            
        # Add 14 days to the 1st Tuesday to get the 3rd Tuesday
        third_tuesday = first_of_month + timedelta(days=days_to_tuesday + 14)
        
        if third_tuesday >= today:
            results.append({
                "date":     third_tuesday.strftime("%Y-%m-%d"),
                "display":  third_tuesday.strftime("%A, %B %d, %Y").replace(" 0", " "),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape Kalamazoo Historic District Commission meetings.",
...
def main() -> None:
    ...
    print(f"HDC Scraper [{mode} mode]")
    print(f"Window: {start_iso} -> {end_iso}\n")

    # ── 1. CivicClerk meetings ──────────────────────────────────────────────
    ...
    hdc_events = [e for e in all_events if is_hdc_event(e)]
    print(f"  HDC events found: {len(hdc_events)}\n")

    scraped_meetings = [m for m in (transform_civicclerk_event(e) for e in hdc_events) if m is not None]
    ...
    # ── 3. Match recordings to meetings ─────────────────────────────────────
    ...
    print("── Step 4: Merging with existing data/hdc.json...")
    ...
    # ── 5. Recompute upcoming meetings ──────────────────────────────────────
    print("── Step 5: Computing upcoming meetings (3rd Tuesday)...")
